...
public interface TransactionRepository extends JpaRepository<Transaction, Long>  {
	...
	@Query(value = "SELECT * FROM transaction WHERE user = ?1 AND MONTH(date) = ?2 AND YEAR(date) = ?3 ORDER BY date DESC", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM transaction WHERE user = ?1 AND YEAR(date) = ?2 AND MONTH(date) = ?3 ORDER BY date DESC", nativeQuery = true)
